...
public abstract class SWActor extends Actor<SWActionInterface> implements SWEntityInterface {
	...
	public SWActor(Team team, int hitpoints, MessageRenderer m, SWWorld world) {
		...
		//An Entity with attack Affordance can be attacked. 
		SWAffordance attack = new Attack(this, m);
		this.addAffordance(attack);
		
		//SWActors are given the Train affordance hence they can be train
		//An Entity with train Affordance can be trained. 
		SWAffordance train = new Train(this, m);
		this.addAffordance(train);
	...
	/**
	 * abstract method that would be implemented in the child class (BenKenobi, Player...)
	 * @return
	 */
	public abstract SWForce getForce();
