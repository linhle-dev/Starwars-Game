...
public class LightSaber extends SWEntity {
	...
	public LightSaber(MessageRenderer m ) {
		...
		this.addAffordance(new Leave(this,m));//add the leave affordance so that it can be put down
		// do if else here like if larger than 100 , then it can pick it up
	 ...
	 * @author Linh
	 * @author Justin
	 * to check is the actor has the enough amount of Force to use a lightsaber
	 * check if actor has force or not (if (temp != null))
	 * check an actors forceStat with the base (>=100) if larger then actor can wield
	 * @return boolean result. True for enough, false for not enough
	 */
	public boolean canWield(SWActor actor) {
		SWForce temp = actor.getForce();
		if (temp != null){
			if (temp.getForceStat() >= temp.getBaseForceStat()) {
				return true;
			}
		}
		return false;
