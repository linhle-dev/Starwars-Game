		...
		if (targetIsActor && (a.getTeam() == targetActor.getTeam())) { //don't attack SWActors in the same team
			...
			if (itemCarried != null) {//if the actor is carrying an item 
				if (itemCarried.hasCapability(Capability.WEAPON)) {
					...
					// check if item is a lightsaber
					if (itemCarried instanceof LightSaber) {

				
						LightSaber lightSaber = (LightSaber) itemCarried;
						// check if lightsaber.canWield(a)
						if (lightSaber.canWield(a)){
							// if yes perform the attack
						...
						}else { // attack with bare hands because actor cannot wield the lightsaber
								// and actor cannot be attacking with a none weapon because whether or not they can wield a lightsaber
					...
					else { // added this else to fix the code
						target.takeDamage(itemCarried.getHitpoints() + 1); // blunt weapon won't do much, but it will still do some damage
						itemCarried.takeDamage(1); // weapon gets blunt
						a.takeDamage(energyForAttackWithWeapon); // actor uses energy to attack
					}
			...
			//targetAcotr!=null is used to keep that target within the map instead of disappear, this will be used in the reservoir situation
			if (this.getTarget().getHitpoints() <= 0 && targetActor!=null) {  // can't use isDead(), as we don't know that the target is an actor
